...
- ~~Header badges could show stale values after a client-side `router.push`~~ — **fixed**: `SummaryView.tsx`/`DefeatView.tsx`'s "Return to Hub" button now calls `router.refresh()` alongside `router.push('/hub')`, since the shared `(game)` layout isn't otherwise re-fetched on sibling navigation under Next.js App Router's default caching. Verified live: a full clear that earned 20 gold + 2 scrap showed the header updating 55→75 gold and scrap 1→2/1 immediately on "Return to Hub", no reload needed. If any other page mutates currency/scrap and navigates away without going through `router.refresh()`, apply the same fix there.
