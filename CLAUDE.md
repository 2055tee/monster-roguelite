...
The Shop + Reforge + Hub navigation redesign project (see "Shipped: Shop + Reforge + Hub navigation redesign" below) is fully complete, deployed locally and live-verified end-to-end on the demo account (WP9–WP15, all 7 work packages). Nothing is currently in progress. Candidate next steps (none started, none requested yet — check with the user before picking one):
...
- The shop/reforge/scrap commits (WP9–WP15) are committed locally but **not yet pushed to `master`** — pushing auto-deploys to production, so confirm with the user before pushing.
...
- Game-engine logic (`src/lib/game/*.ts`, excluding `types.ts`/`constants.ts`/`seed-data.ts`) is pure and I/O-free, unit-tested with vitest (59 tests). Safe to extend — run `npm test` after changes.
...
- Run both before every deploy. `npm test` is at 59 unit tests (up from 37) as of WP9; `tests/loop.ts` is at 73 assertions (up from 50) as of WP13; both green as of the last commit.
...
- **Header badges (`CurrencyBadge`/`ScrapBadge` in `layout.tsx`) can show stale values after a client-side `router.push`** (e.g. clicking "Return to Hub" from the run-summary screen) — the shared `(game)` layout isn't re-fetched on sibling navigation under Next.js App Router's default caching, so a `SummaryView`-reported gold/scrap gain doesn't show in the header until a hard reload or full navigation. Confirmed live during WP15: header showed 95g/hard-reload showed the correct 115g. Pre-existing behavior (affects `CurrencyBadge`, not something WP13 introduced with `ScrapBadge`) — worth fixing with a `router.refresh()` on return-to-hub or a differently-scoped fetch if it becomes annoying, but not blocking since every game-state page already calls `getHubState()`/`ensureProfile()` fresh on its own load.
- The Shop + Reforge project's commits are on local `master` but have **not been pushed** — remember pushing triggers Vercel auto-deploy to production.
