...
- `npx tsx --env-file=.env.local tests/loop.ts` — integration test against the **live** Supabase project. Drives bootstrap → start run → combat → rest → boss → catch → finish, plus double-start rejection, run-abandon, XP-award/level-up/idempotency, item-instance/equip/reforge (WP10), and shop-purchase (WP11) checks (62 assertions). This bypasses `requireUser()`'s `next/headers` dependency by calling the same repo/game-bridge layer the actions delegate to, with a resolved user id passed in directly — see the file's header comment for why.
- Run both before every deploy. `npm test` is at 59 unit tests (up from 37) as of WP9; `tests/loop.ts` is at 62 assertions (up from 50) as of WP11; both green as of the last commit.
