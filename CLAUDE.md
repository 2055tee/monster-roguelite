...
## ⚠️ START HERE: P0 combat bug fixed, needs a fresh full click-through to confirm boss winnability
...
**Fix applied:** `'basic_attack'` is now always included when a `Combatant` is constructed (`game-bridge.ts`'s `buildPlayerCombatant` and `combat.ts`'s `buildEnemy`), and `CombatView.tsx` no longer prepends it separately (`activeCombatant.abilities.map(...)` instead of `['basic_attack', ...activeCombatant.abilities].map(...)`). Verified: `npx tsc --noEmit` clean, `npm test` 20/20, `tests/loop.ts` 45/45 against live Supabase. Manually reproduced the original 500 in the browser (confirmed it happened on a **fresh, unguarded** enemy, disproving the Bulwark theory), then reproduced the fix locally-verified-only — **not yet re-verified against the redeployed production build**, since this fix hasn't been pushed/deployed yet as of writing this note.
...
1. `git push` the pending commit (or check if it's already pushed — see git log) so the Vercel git link redeploys with the fixes.
2. Re-run the **full playwright click-through** end-to-end against the redeployed production URL: demo login → hub → dungeon run → combat rooms (confirm Strike/basic_attack now works) → rest rooms → boss → catch attempt → finish → confirm hub/roster/inventory reflect it correctly (including the two display fixes).
3. **This is the one open question that still needs a real answer**: can a level-2 starter team, played well (not the `tests/loop.ts` scripted-AI shortcut, which deterministically weakens the boss to 1 HP purely to test catch/finish wiring — see WP3 notes below), actually beat the Verdant Hollow boss (Thornmaw)? Answer it definitively this time now that combat itself works.
4. Only once that's clean, write the final CLAUDE.md documentation pass (replacing this session-state file with real project docs) — the last unchecked row in the table above.
