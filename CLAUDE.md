...
**In progress: Shop + Reforge + Hub navigation redesign** (see that section below for the full locked spec — WP9–WP15). Planned by an Opus pass, decisions confirmed with the user, being implemented by Sonnet one work package at a time, same process as the earlier XP/UI project. WP9–WP14 are done and live-verified. **Only WP15 remains** (docs rewrite of that section into "Shipped" + one full end-to-end production playthrough exercising everything together — see WP15's description for the exact checklist). Whichever session picks this up should do WP15 and nothing else; every formula/price/UI decision for WP9-14 is already implemented, not just planned.
