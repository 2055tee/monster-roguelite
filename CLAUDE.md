...
- **Not yet tried**: using the starter Lure Bait / any consumables (a Field Elixir instant-heal exists per the item table but wasn't confirmed in the starter grant or used this attempt), or simply playing more conservatively (e.g. always healing before offense). Worth a second attempt with consumables in hand before concluding this needs a balance patch — but as observed, a "played reasonably, no items" run does not survive to the boss.
...
1. **Decide on the balance question above** — either (a) tune Verdant Hollow room 1 enemy damage down and/or the room layout to give a rest before room 2, or (b) confirm consumables (Lure Bait, Field Elixir) are meant to carry a level-2 team through and re-test using them deliberately before concluding it's a balance bug. Check `src/lib/game/seed-data.ts` / the `items` table for what the starter grant actually includes.
2. Once (1) is resolved, do one more **fully isolated** playwright pass (no other agent/session touching the same browser mid-run) all the way through: dungeon run → combat rooms (using consumables if that's the intended path) → rest rooms → boss → catch attempt → finish → confirm hub/roster/inventory reflect it correctly. This is the pass that finally answers boss winnability definitively.
...
4. Only once boss winnability (or a deliberate balance fix) is confirmed, write the final CLAUDE.md documentation pass (replacing this session-state file with real project docs) — the last unchecked row in the table above.
