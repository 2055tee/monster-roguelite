...
| WP6 | Deploy to Vercel (git-linked project, env vars, Supabase auth redirect URL) + full playwright click-through verification | ⚠️ **Deployed, P0 combat bug fixed and confirmed. A clean isolated playthrough now shows a likely balance problem, not a bug** — see below |
| — | Write final CLAUDE.md docs | ❌ Not started (do this once the balance question below is resolved one way or another) |

## ⚠️ START HERE: combat itself works; a clean playthrough suggests Verdant Hollow room 1→2 is undertuned for a level-2 team
...
**Boss winnability — now has a real (if unwelcome) answer, from a third, fully isolated playthrough:** the level-2 starter team (Sprigling/Cinderpup/Pebblet) was wiped in **room 2**, never reaching the boss. Played reasonably (focus-fired the lower-HP enemy each turn, used Mend/Bulwark, prioritized Heavy Blow/Venom Fang over plain Strike when off cooldown). What happened:
- Room 1 (two Lv3 enemies) alone cost the team: Sprigling fainted after a single 27-damage hit from Zaplet's Heavy Blow (57% of Sprigling's 47 max HP in one blow), and Cinderpup was left at 1 HP by the room's end even after a Mend.
- **There is no rest room between room 1 and room 2** (layout is `[combat, combat, rest, combat, rest, boss]` — the first rest isn't until room 3), so the team entered room 2 already down one monster and another at 1 HP, against two more Lv3 enemies (2v1 effectively). The surviving Pebblet got focus-fired by both enemies and the team wiped a few rounds in.
- This is a **clean, correct loss** — no crash, no bug: proper "Your Team Was Defeated" screen, run marked failed, team returned to hub at 0 HP each in the (by-design) healing state, dungeon selection unlocked again. The mechanics all worked; the numbers just didn't favor the player.
- Read as a balance signal, not a wiring bug: either enemy damage in Verdant Hollow (nominally the *easiest*, Tier 1 dungeon, enemies at Lv3 vs. a Lv2 team) is tuned too high relative to starter HP pools, or the fixed `[combat, combat, rest, ...]` layout front-loads too much unhealed risk before the first rest room, or both. `tests/loop.ts`'s scripted AI never surfaces this because it deterministically weakens the boss to 1 HP and its simple greedy AI/room sequencing may not hit the same worst-case room-1 damage rolls.
- **Consumables checked and ruled out as a fix**: the demo account's starter grant is 1 Lure Bait (`+15pp catch chance` — no combat use). `Field Elixir` (`instant_heal`) exists in the item table, but per `src/server/actions/hub.ts` (`useFieldElixir`, ~line 152) it only skips the **out-of-combat hub healing timer** — there is no consumable, ability, or mechanic that heals mid-run between rooms 1 and 2 other than Mend/Bulwark (already used in the losing attempt). **This confirms it's a genuine balance/design gap, not a missed-item oversight**: a level-2 team has no safety valve for the unhealed room-1→room-2 stretch as currently tuned.
...
**Also added this session:** ability tooltips — hovering a skill button in combat now shows what it does (native `title` attribute), via a new `description` field on each entry in `src/lib/game/abilities.ts`, wired up in `CombatView.tsx`.

**Confirmed by-design (not a bug) while playing:** a fainted ally cannot be targeted by Mend mid-combat — `CombatView.tsx`'s `renderRow` only wires `onClick` for non-dead targets, so there's no mid-fight revive; fainted monsters only recover via the out-of-hub healing-timer mechanic. Consistent with "no permadeath, but fainted = sidelined until healed."

**Next session should, in order:**
1. **Pick a balance fix** (asked the user this session, awaiting their call as of writing): (a) reduce Verdant Hollow room-1 enemy damage and/or (b) change the room layout so there's a rest before room 2 (e.g. `[combat, rest, combat, rest, combat, boss]`), and/or (c) grant a combat-usable heal item in the starter kit if that's meant to be the intended safety valve instead. `src/lib/game/seed-data.ts` (dungeon room layouts, enemy stats) and `src/lib/game/constants.ts` (`ROOM_LAYOUT_DEFAULT`) are the places to change this.
2. Once a fix is chosen and applied, do one more **fully isolated** playwright pass (no other agent/session touching the same browser mid-run) all the way through: dungeon run → combat rooms → rest rooms → boss → catch attempt → finish → confirm hub/roster/inventory reflect it correctly. This is the pass that finally answers boss winnability definitively.
3. Consider parallelizing the sequential DB `await`s in `src/server/actions/{combat,hub,run}.ts` with `Promise.all` where calls are independent — the region pin (`sin1`) fixes the geography, not the serialization.
4. Only once boss winnability (post-balance-fix) is confirmed, write the final CLAUDE.md documentation pass (replacing this session-state file with real project docs) — the last unchecked row in the table above.
